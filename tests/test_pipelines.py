...
class DownloadTests(unittest.TestCase):
    ...
    def test_download_only_pytorch(self):
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            all_root_files = [t[-1] for t in os.walk(os.path.join(
                tmpdirname, os.listdir(tmpdirname)[0], 'snapshots'))]
    ...
    def test_cached_files_are_used_when_no_internet(self):
        ...
        orig_pipe = StableDiffusionPipeline.from_pretrained(
            'hf-internal-testing/tiny-stable-diffusion-torch',
            safety_checker=None)
        orig_comps = {k: v for k, v in orig_pipe.components.items() if
            hasattr(v, 'parameters')}
        ...
        for safe_avail in [False, True]:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                StableDiffusionPipeline.from_pretrained(
                    'hf-internal-testing/stable-diffusion-all-variants',
                    cache_dir=tmpdirname, variant=variant)
                snapshots = os.path.join(tmpdirname, os.listdir(tmpdirname)
                    [0], 'snapshots')
                all_root_files = [t[-1] for t in os.walk(snapshots)]
                files = [item for sublist in all_root_files for item in sublist
                    ]
                unet_files = os.listdir(os.path.join(snapshots, os.listdir(
                    snapshots)[0], 'unet'))
                assert len(files
                    ) == 15, f'We should only download 15 files, not {len(files)}'
    ...
    def test_download_broken_variant(self):
        for safe_avail in [False, True]:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                pipe = StableDiffusionPipeline.from_pretrained(
                    'hf-internal-testing/stable-diffusion-broken-variants',
                    cache_dir=tmpdirname, variant='fp16')
...
class CustomPipelineTests(unittest.TestCase):
    ...
    def test_load_custom_pipeline(self):
        pipeline = DiffusionPipeline.from_pretrained('google/ddpm-cifar10-32',
            custom_pipeline='hf-internal-testing/ppdiffusers-dummy-pipeline')
        pipeline = pipeline
    ...
    def test_load_pipeline_from_git(self):
        ...
        pipeline = DiffusionPipeline.from_pretrained(
            ...
            feature_extractor=feature_extractor, paddle_dtype=paddle.float16)
...
class PipelineFastTests(unittest.TestCase):
    ...
    def dummy_image(self):
        ...
        num_channels = 3
        sizes = 32, 32
        image = floats_tensor((batch_size, num_channels) + sizes, rng=
            random.Random(0))
        return image
    ...
    @parameterized.expand([[DDIMScheduler, DDIMPipeline, 32], [
        DDPMScheduler, DDPMPipeline, 32], [DDIMScheduler, DDIMPipeline, (32,
        64)], [DDPMScheduler, DDPMPipeline, (64, 32)]])
    ...
    def test_set_scheduler(self):
        ...
        tokenizer = CLIPTokenizer.from_pretrained(
            'hf-internal-testing/tiny-random-clip')
        sd = StableDiffusionPipeline(unet=unet, scheduler=scheduler, vae=
            vae, text_encoder=bert, tokenizer=tokenizer, safety_checker=
        ...
        sd.scheduler = EulerAncestralDiscreteScheduler.from_config(sd.
            scheduler.config)
        assert isinstance(sd.scheduler, EulerAncestralDiscreteScheduler)
        sd.scheduler = DPMSolverMultistepScheduler.from_config(sd.scheduler
            .config)
    ...
    def test_set_scheduler_consistency(self):
        ...
        pndm_config_2 = {k: v for k, v in pndm_config_2.items() if k in
            pndm_config}
        ...
        ddim_config_2 = {k: v for k, v in ddim_config_2.items() if k in
            ddim_config}
    ...
    def test_no_pytorch_download_when_doing_safetensors(self):
        with tempfile.TemporaryDirectory() as tmpdirname:
            _ = StableDiffusionPipeline.from_pretrained(
                'hf-internal-testing/ppdiffusers-stable-diffusion-tiny-all',
                cache_dir=tmpdirname)
            path = os.path.join(tmpdirname,
                'models--hf-internal-testing--ppdiffusers-stable-diffusion-tiny-all'
                , 'snapshots', '07838d72e12f9bcec1375b0482b80c1d399be843',
                'unet')
    ...
    def test_optional_components(self):
        ...
        tokenizer = CLIPTokenizer.from_pretrained(
            'hf-internal-testing/tiny-random-clip')
        orig_sd = StableDiffusionPipeline(unet=unet, scheduler=pndm, vae=
            vae, text_encoder=bert, tokenizer=tokenizer, safety_checker=
            unet, feature_extractor=self.dummy_extractor)
        sd = orig_sd
...
class PipelineSlowTests(unittest.TestCase):
    ...
    def test_smart_download(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            snapshot_dir = os.path.join(tmpdirname, local_repo_name,
                'snapshots')
            snapshot_dir = os.path.join(snapshot_dir, os.listdir(
                snapshot_dir)[0])
            assert os.path.isfile(os.path.join(snapshot_dir,
            ...
            assert os.path.isfile(os.path.join(snapshot_dir, 'scheduler',
                SCHEDULER_CONFIG_NAME))
            assert os.path.isfile(os.path.join(snapshot_dir, 'unet',
                WEIGHTS_NAME))
            assert os.path.isfile(os.path.join(snapshot_dir, 'unet',
                WEIGHTS_NAME))
            assert not os.path.isfile(os.path.join(snapshot_dir,
                'big_array.npy'))
    ...
    def test_from_pretrained_hub(self):
        ...
        image = ddpm(generator=generator, num_inference_steps=5,
            output_type='numpy').images
        generator = paddle.Generator().manual_seed(0)
        new_image = ddpm_from_hub(generator=generator, num_inference_steps=
            5, output_type='numpy').images
        assert np.abs(image - new_image).sum(
            ) < 1e-05, "Models don't give the same forward pass"
...
class PipelineNightlyTests(unittest.TestCase):
    ...
    def test_ddpm_ddim_equality_batched(self):
        ...
        ddpm_images = ddpm(batch_size=2, generator=generator, output_type=
            'numpy').images
        generator = paddle.Generator().manual_seed(seed)
        ddim_images = ddim(batch_size=2, generator=generator,
            num_inference_steps=1000, eta=1.0, output_type='numpy',
