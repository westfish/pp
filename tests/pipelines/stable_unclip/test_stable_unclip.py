...
from ppdiffusers.utils.testing_utils import load_numpy, require_paddle_gpu, slow
...
class StableUnCLIPPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def get_dummy_components(self):
        ...
        prior_text_encoder = CLIPTextModelWithProjection(CLIPTextConfig(
            bos_token_id=0, eos_token_id=2, hidden_size=
            embedder_hidden_size, projection_dim=embedder_projection_dim,
            intermediate_size=37, layer_norm_eps=1e-05, num_attention_heads
            =4, num_hidden_layers=5, pad_token_id=1, vocab_size=1000))
        paddle.seed(0)
        prior = PriorTransformer(num_attention_heads=2, attention_head_dim=
            12, embedding_dim=embedder_projection_dim, num_layers=1)
        paddle.seed(0)
        ...
        image_noising_scheduler = DDPMScheduler(beta_schedule=
            'squaredcos_cap_v2')
        paddle.seed(0)
        ...
        text_encoder = CLIPTextModel(CLIPTextConfig(bos_token_id=0,
            eos_token_id=2, hidden_size=embedder_hidden_size,
        ...
        unet = UNet2DConditionModel(sample_size=32, in_channels=4,
            ...
            'CrossAttnUpBlock2D'), block_out_channels=(32, 64),
            attention_head_dim=(2, 4), class_embed_type='projection',
            projection_class_embeddings_input_dim=embedder_projection_dim *
            2, cross_attention_dim=embedder_hidden_size, layers_per_block=1,
        ...
        paddle.seed(0)
        vae = AutoencoderKL()
        components = {'prior_tokenizer': prior_tokenizer,
            'prior_text_encoder': prior_text_encoder, 'prior': prior,
            'prior_scheduler': prior_scheduler, 'image_normalizer':
            image_normalizer, 'image_noising_scheduler':
            image_noising_scheduler, 'tokenizer': tokenizer, 'text_encoder':
    ...
    def get_dummy_inputs(self, seed=0):
        ...
        inputs = {'prompt': 'A painting of a squirrel eating a burger',
            ...
            'prior_num_inference_steps': 2, 'output_type': 'numpy'}
        return inputs
