...
class UnCLIPPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def dummy_text_encoder(self):
        ...
        config = CLIPTextConfig(bos_token_id=0, eos_token_id=2, hidden_size
            ...
            =1e-05, num_attention_heads=4, num_hidden_layers=5,
    ...
    def dummy_text_proj(self):
        paddle.seed(0)
        model_kwargs = {'clip_embeddings_dim': self.
            text_embedder_hidden_size, 'time_embed_dim': self.
            time_embed_dim, 'cross_attention_dim': self.cross_attention_dim}
    ...
    def dummy_decoder(self):
        paddle.seed(0)
        model_kwargs = {'sample_size': 32, 'in_channels': 3, 'out_channels':
            6, 'down_block_types': ('ResnetDownsampleBlock2D',
            ...
            'attention_head_dim': 4, 'resnet_time_scale_shift':
            'scale_shift', 'class_embed_type': 'identity'}
        model = UNet2DConditionModel(**model_kwargs)
        return model

    @property
    def dummy_super_res_kwargs(self):
        return {'sample_size': 64, 'layers_per_block': 1,
            'down_block_types': ('ResnetDownsampleBlock2D',
            'ResnetDownsampleBlock2D'), 'up_block_types': (
    ...
    def get_dummy_components(self):
        ...
        super_res_scheduler = UnCLIPScheduler(variance_type=
            'fixed_small_log', prediction_type='epsilon',
    ...
    def get_dummy_inputs(self, seed=0):
        ...
        inputs = {'prompt': 'horse', 'generator': generator,
            'prior_num_inference_steps': 2, 'decoder_num_inference_steps': 
            2, 'super_res_num_inference_steps': 2, 'output_type': 'numpy'}
        return inputs
    ...
    def test_unclip_passed_text_embed(self):
        ...
        output = pipe([prompt], generator=generator,
            ...
            super_res_num_inference_steps=2, prior_latents=prior_latents,
        ...
        generator = paddle.Generator().manual_seed(0)
        image_from_text = pipe(generator=generator,
            prior_num_inference_steps=2, decoder_num_inference_steps=2,
            super_res_num_inference_steps=2, prior_latents=prior_latents,
            decoder_latents=decoder_latents, super_res_latents=
            super_res_latents, text_model_output=text_model_output,
            text_attention_mask=text_attention_mask, output_type='np')[0]
