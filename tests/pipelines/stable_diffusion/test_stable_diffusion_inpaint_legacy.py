...
class StableDiffusionInpaintLegacyPipelineFastTests(unittest.TestCase):
    ...
    def test_stable_diffusion_inpaint_legacy_negative_prompt(self):
        ...
        generator = paddle.Generator().manual_seed(0)
        output = sd_pipe(prompt, negative_prompt=negative_prompt, generator
    ...
    def test_stable_diffusion_inpaint_legacy_num_images_per_prompt(self):
        ...
        images = sd_pipe(prompt, num_inference_steps=2, output_type='np',
            image=init_image, mask_image=mask_image).images
        assert images.shape == (1, 32, 32, 3)
        batch_size = 2
        images = sd_pipe([prompt] * batch_size, num_inference_steps=2,
            output_type='np', image=init_image, mask_image=mask_image).images
        assert images.shape == (batch_size, 32, 32, 3)
        num_images_per_prompt = 2
        images = sd_pipe(prompt, num_inference_steps=2, output_type='np',
            image=init_image, mask_image=mask_image, num_images_per_prompt=
...
class StableDiffusionInpaintLegacyPipelineSlowTests(unittest.TestCase):
    ...
    def get_inputs(self, device, generator_device='cpu', dtype='float32',
        ...
        inputs = {'prompt': 'A red cat sitting on a park bench', 'image':
            init_image, 'mask_image': mask_image, 'generator': generator,
            'num_inference_steps': 3, 'strength': 0.75, 'guidance_scale': 
            7.5, 'output_type': 'numpy'}
        return inputs
...
class StableDiffusionInpaintLegacyPipelineNightlyTests(unittest.TestCase):
    ...
    def get_inputs(self, device, generator_device='cpu', dtype='float32',
        ...
        inputs = {'prompt': 'A red cat sitting on a park bench', 'image':
            init_image, 'mask_image': mask_image, 'generator': generator,
            'num_inference_steps': 50, 'strength': 0.75, 'guidance_scale': 
            7.5, 'output_type': 'numpy'}
        return inputs
    ...
    def test_inpaint_pndm(self):
        ...
        expected_image = load_numpy(
            'https://huggingface.co/datasets/ppdiffusers/test-arrays/resolve/main/stable_diffusion_inpaint_legacy/stable_diffusion_1_5_pndm.npy'
            )
    ...
    def test_inpaint_ddim(self):
        ...
        image = sd_pipe(**inputs).images[0]
        expected_image = load_numpy(
            'https://huggingface.co/datasets/ppdiffusers/test-arrays/resolve/main/stable_diffusion_inpaint_legacy/stable_diffusion_1_5_ddim.npy'
            )
        max_diff = np.abs(expected_image - image).max()
        assert max_diff < 0.001
    ...
    def test_inpaint_lms(self):
        ...
        image = sd_pipe(**inputs).images[0]
        expected_image = load_numpy(
            'https://huggingface.co/datasets/ppdiffusers/test-arrays/resolve/main/stable_diffusion_inpaint_legacy/stable_diffusion_1_5_lms.npy'
            )
        max_diff = np.abs(expected_image - image).max()
        assert max_diff < 0.001
    ...
    def test_inpaint_dpm(self):
        ...
        image = sd_pipe(**inputs).images[0]
        expected_image = load_numpy(
            'https://huggingface.co/datasets/ppdiffusers/test-arrays/resolve/main/stable_diffusion_inpaint_legacy/stable_diffusion_1_5_dpm_multi.npy'
            )
        max_diff = np.abs(expected_image - image).max()
        assert max_diff < 0.001
